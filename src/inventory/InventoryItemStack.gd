...
## This number is always in the range [1, item_data.stack_size).
var count: int:
	set = _set_item_count;
...
func _set_item_count(new_count: int) -> void:
	if new_count <= 0 || new_count > data.stack_size:
		push_error("Attempted to set item count to value outside of item's range (%s -> [1, %s] for item with id %s))" % [new_count, data.stack_size, data.id]);
		print_stack();
		count = clamp(new_count, 1, data.stack_size);
	else:
		count = new_count;
